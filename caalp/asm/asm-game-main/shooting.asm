...
data segment para 'DATA'
	...
	plr_sz dw 08h
	plr_velo dw 06h                            ; Reduced speed for smoother control
	...
	game_fin db 'Game Finished - Press Any Key $'
	startscreen db 'WASD=Move | IJKL=Shoot | Press Any Key $'
...
code segment para 'CODE'
	...
	check_shoot proc near
		...
		cont3:
		; Check both uppercase and lowercase for shooting
		cmp AL, 69h                                      ;if i is pressed -> blt dir up
		je i_pressed
		cmp AL, 49h                                      ;if I is pressed -> blt dir up
		je i_pressed

		cmp AL, 6Bh                                      ;if k is pressed -> blt dir down
		je k_pressed
		cmp AL, 4Bh                                      ;if K is pressed -> blt dir down
		je k_pressed

		cmp AL, 6Ah                                      ;if j is pressed -> blt dir left
		je j_pressed
		cmp AL, 4Ah                                      ;if J is pressed -> blt dir left
		je j_pressed

		cmp AL, 6Ch                                      ;if l is pressed -> blt dir right
		je l_pressed
		cmp AL, 4Ch                                      ;if L is pressed -> blt dir right
		je l_pressed
		ret
	...
	mv_plr proc near
		...
		; Check WASD with scan codes (AH) - MORE RELIABLE
		cmp AH, 11h   ; W scan code
		je w_pressed

		cmp AH, 1Fh   ; S scan code
		je s_pressed

		cmp AH, 1Eh   ; A scan code
		je a_pressed

		cmp AH, 20h   ; D scan code
		je d_pressed

		; Also check ASCII codes (AL) as backup
		cmp AL, 77h   ; w lowercase
		je w_pressed
		cmp AL, 57h   ; W uppercase
		je w_pressed

		cmp AL, 73h   ; s lowercase
		je s_pressed
		cmp AL, 53h   ; S uppercase
		je s_pressed

		cmp AL, 61h   ; a lowercase
		je a_pressed
		cmp AL, 41h   ; A uppercase
		je a_pressed

		cmp AL, 64h   ; d lowercase
		je d_pressed
		cmp AL, 44h   ; D uppercase
		je d_pressed
		...
		w_pressed:
			cmp plr_y, 10h                                  ;checks if plr_y < 10h
			jc end4

			mov AX, plr_velo                              ;mvs plr_y by plr_velo
			sub plr_y, AX
			jmp end4
		...
		a_pressed:
			cmp plr_x, 10h                                  ;checks if plr_x < 10h
			jc end4
			mov AX, plr_velo                              ;mvs plr_x by plr_velo
			sub plr_x, AX
			jmp end4
	...
	clrscr proc near    ;clear screen
		...
		mov CX, 07D00h   ; Store the total number of bytes required for mode 13h (320x200)
		mov AX, 0h
	...
	draw_blts proc near
		...
		draw_blt_hz:
			...
			mov AH, 0ch
			mov AL, 0Eh						;color yellow (bright)
			mov BH, 00h
			int 10h
